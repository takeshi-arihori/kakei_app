...
- GroupRepositoryはload / commit / findOperationという分析文書の契約とする。compare-and-swapで状態・版・必要履歴・operation結果を原子的に扱う。個別Participant／OwnerのRepositoryは公開しない。
...
実装済みDataはAcceptance時点に存在しない。採用後に変更が必要なら本ADRを直接書き換えず、新ADRで互換性・移行を決める。再参加、4人超、Owner複数化、在籍履歴肥大化、Group終了との競合がReview Trigger。
