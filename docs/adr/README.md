...
| [CloseIntentIdの保持期間と一意性境界](close-intent-id-retention-boundary.md) | Accepted | [ADR #102](https://github.com/takeshi-arihori/kakei_app/issues/102)、Amends #55／#73、[Task #96](https://github.com/takeshi-arihori/kakei_app/issues/96)、[Task #103](https://github.com/takeshi-arihori/kakei_app/issues/103) |
| [Retention削除とGroup Management Receiptの順序](retention-deletion-receipt-order.md) | Proposed（Owner decision required） | [ADR Proposal #115](https://github.com/takeshi-arihori/kakei_app/issues/115)、Amends proposal #55／#102 |
...
- [CloseIntentIdの保持期間と一意性境界](close-intent-id-retention-boundary.md): Accepted。#55／#73を一部改訂し、registryをGroup削除後1年保持する。期限後もcleanup commitまでは重複を拒否し、cleanup commit後はCSPRNG UUIDv4の実用上一意性に依存する。#96と#103はこのDecisionへ整合する。
- [Retention削除とGroup Management Receiptの順序](retention-deletion-receipt-order.md): Proposed。GMのpost-deletion receiptが全Context receipt gateと循環する問題に対し、Prepared fence → Expense Recording／Settlement receipt → GM deletion transaction／receipt → all-receipt verification／Completeの二段階順序を提案する。OwnerがAcceptedと記録するまで#55／#102は変更されず、#103とProduction deletionはBlockedのまま。
