...
- 未認証、本人不一致、他Group参照、不正招待は状態・履歴・operation結果を作らない。外部応答は非在籍者にGroup存在を漏らさないUnavailableToActorへ統一する。内部のNotFound／Forbiddenとの変換は本番接続前に定義する。
...
- 成功済みCommand結果の再取得は現在のOwner権限が失われても同じActorに限り許す。返却は当該結果のID・versionだけで現在Groupの参照権限は付与しない。
- 未記録／失敗後の操作は最新状態で再認可する。Conflictで変更した入力・期待版を使う場合は新operationId。成功したか不明なUnavailableは同一operationを照合する。失敗結果は記録しない。
...
本人脱退のみ、認可版の固定、成功結果を同一Actorへ最小限再取得できる例外、失敗結果を保存しない方針を案Aとして採用する。依存Application TaskはAccepted記録の`develop`統合後に個別Ready評価する。内部fake検証を本番認証・保存の証拠にしない。
...
Acceptance時点で本方式の実装済みDataは存在しない。採用後の方式変更は本ADRを直接書き換えず新ADRへ分離する。認証Provider採用、招待・再参加、本番接続、退会／削除、跨Context操作、監査要件の発生をReview Triggerとする。
