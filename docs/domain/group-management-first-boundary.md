...
- 条件C1は充足済み。Group終了のRepository同期と正式Security Reviewを完了し、S0の#75はReady評価済み。Persistence実装はS0〜S3が完了するまで段階的に進め、#42はBacklog / Blocked Yesを維持する。以下のPortはApplication契約としてAcceptedだが、本番保存Adapterは#42で個別Ready評価する。
