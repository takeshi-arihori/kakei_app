...
- C1充足だけではPersistence TaskをReadyにしない。ADR #73のRepository同期と正式Security Review passを確認し、S0の#75はReady評価済み。#74（S1）と#77（S2）はDoneで、#75（S0）完了後に#76（S3）を個別Ready評価し、S0〜S3完了後に#42を個別Ready評価する。実Context fence／Receipt配送、Key Provider、Audit Store、Retention／Backup Gateが揃うまで本番へwireしない。
